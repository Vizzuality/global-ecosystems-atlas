  ...
  return (
    <div className="space-y-4 pt-1.5">
      <div className="h-4">
        <ParentSize>
          {({ width, height }) => (
            <BiomesExtentChart width={width} height={height} location={location} />
          )}
        </ParentSize>
      </div>

      <BiomesExtentRanking location={location} />
    </div>
  );
};

export const BiomesExtentChart = ({
  width,
  height,
  location,
}: {
...
export const BiomesExtentRanking = ({ location }: { location: string }) => {
  const { data } = useApiLocationsLocationWidgetsWidgetIdGet(location ?? "GLOB", "extent_biomes");
